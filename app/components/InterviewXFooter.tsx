...
import { publicAssetUrl, workforcePath } from "@/lib/apiBase";
import {
  buildInterviewXManagerDashboardUrlWithSso,
  buildInterviewXStudentPrepHomeUrlWithSso,
} from "@/lib/interviewx";
...
type PlatformLink = { href: string; label: string; external?: boolean };

const GUEST_PLATFORM_LINKS: PlatformLink[] = [
  { href: "/", label: "Home" },
  { href: "/role/", label: "TalentX Roles" },
  { href: "/dashboard/manager", label: "Monitoring" },
  { href: "/dashboard/manager/interviews", label: "Technical Assesments" },
];

const COMPANY_LINKS_TOP = [{ href: "/auth/manager/login", label: "Request Demo" }];
const COMPANY_LINKS_REST = [
  { href: "/industry", label: "Industries" },
  { href: "/education", label: "Education" },
];

function isManagerOrHr(user: SupportAuth["user"]): boolean {
  return Boolean(user && user.accountType === "EMPLOYEE" && (user.currentRole === "MANAGER" || user.currentRole === "HR"));
}

function buildPlatformLinks(auth: SupportAuth): PlatformLink[] {
  if (!auth.token || !auth.user) {
    return GUEST_PLATFORM_LINKS.map((l) => ({ ...l, href: workforcePath(l.href) }));
  }

  const homeHref = workforcePath("/");

  if (isManagerOrHr(auth.user)) {
    return [
      { href: homeHref, label: "Home" },
      { href: workforcePath("/dashboard/manager"), label: "TalentX" },
      {
        href: buildInterviewXManagerDashboardUrlWithSso({
          token: auth.token,
          email: auth.user.email,
          name: auth.user.fullName,
          userType: auth.user.currentRole === "HR" ? "HR" : "MANAGER",
        }),
        label: "InterviewX",
        external: true,
      },
  ...
  if (auth.user.accountType === "EMPLOYEE") {
    return [
      { href: homeHref, label: "Home" },
      { href: workforcePath("/target-role"), label: "Role blueprint" },
      {
        href: buildInterviewXStudentPrepHomeUrlWithSso({
          token: auth.token,
          email: auth.user.email,
          name: auth.user.fullName,
        }),
...
function PlatformLinkItem({ item }: { item: PlatformLink }) {
  if (item.external) {
    return (
      <a href={item.href} className="ix-site-footer__link">
        {item.label}
      </a>
    );
  }
...
export function InterviewXFooter() {
  ...
  useEffect(() => {
    const refresh = () => setAuth(resolveSupportAuth());
    refresh();
    window.addEventListener("hs-auth-changed", refresh);
    window.addEventListener("jbv2-org-auth-changed", refresh);
    window.addEventListener("storage", refresh);
    return () => {
      window.removeEventListener("hs-auth-changed", refresh);
      window.removeEventListener("jbv2-org-auth-changed", refresh);
      window.removeEventListener("storage", refresh);
    };
  }, []);

  const platformLinks = useMemo(() => buildPlatformLinks(auth), [auth]);
        ...
        <div className="ix-site-footer__inner">
          <div className="ix-site-footer__grid">
            <div className="ix-site-footer__col ix-site-footer__col--brand">
              <div className="ix-site-footer__brand-panel">
                <img
                  src={logoSrc}
                  alt="SX Workforce"
            ...
            <div className="ix-site-footer__col">
              ...
              <ul className="ix-site-footer__links">
                {COMPANY_LINKS_TOP.map((l) => (
                  <li key={l.href}>
                    <a href={workforcePath(l.href)} className="ix-site-footer__link">
                      {l.label}
                    </a>
                  </li>
                ))}
                <li>
                  <a href={workforcePath("/role/")} className="ix-site-footer__link">
                    Get Started
                  </a>
                </li>
